...
modules_for_hpc
nohup sshpass -p 3tango ssh root@$HNAME 'reboot' > /dev/null 2>&1 &

echo "Waiting for server to reboot..."
sleep 300

until sshpass -p 3tango ssh -o ConnectTimeout=10 -o ConnectionAttempts=1 root@$HNAME true; do
...
sshpass -p 3tango ssh root@$HNAME 'env node=compute site=MTR sync_slurm=yes sync_nhc=yes restart_slurm=yes \
